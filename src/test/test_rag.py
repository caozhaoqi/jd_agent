...
async def test_query_knowledge_base_normal(
    mock_ask_knowledge_base,
    client: TestClient,
    test_token: str,
    mock_rag_question,
    mock_rag_answer,
...
@patch("app.api.routers.rag.ask_knowledge_base", new_callable=AsyncMock)
async def test_query_knowledge_base_empty_question(
    mock_ask_knowledge_base, client: TestClient, test_token: str
):
    """测试空查询问题"""
    # 创建空查询请求
...
@patch("app.api.routers.rag.ask_knowledge_base", new_callable=AsyncMock)
async def test_query_knowledge_base_value_error(
    mock_ask_knowledge_base, client: TestClient, test_token: str, mock_rag_question
):
    """测试查询时发生ValueError错误"""
    # 配置模拟对象抛出ValueError
...
@patch("app.api.routers.rag.ask_knowledge_base", new_callable=AsyncMock)
async def test_query_knowledge_base_exception(
    mock_ask_knowledge_base, client: TestClient, test_token: str, mock_rag_question
):
    """测试查询时发生其他异常"""
    # 配置模拟对象抛出Exception
...
@patch("app.api.routers.rag.ask_knowledge_base", new_callable=AsyncMock)
async def test_query_knowledge_base_no_sources(
    mock_ask_knowledge_base, client: TestClient, test_token: str, mock_rag_question
):
    """测试没有来源的查询结果"""
    # 配置模拟对象返回没有来源的结果
