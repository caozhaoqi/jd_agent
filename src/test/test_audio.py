...
async def test_transcribe_audio(
    mock_openai, client: TestClient, test_token: str, mock_audio_data
):
    """测试语音转文字接口"""
    # 配置模拟对象
...
async def test_transcribe_audio_empty_file(
    mock_openai, client: TestClient, test_token: str
):
    """测试上传空音频文件"""
    # 配置模拟对象
    mock_transcript = MagicMock()
...
async def test_text_to_speech_mac(
    mock_open,
    mock_remove,
    mock_getsize,
    mock_exists,
...
@patch("os.remove")
@patch("builtins.open", new_callable=MagicMock)
async def test_text_to_speech_windows(
    mock_open,
    mock_remove,
    mock_getsize,
    mock_exists,
...
async def test_text_to_speech_empty_text(
    mock_remove, mock_audiosegment, mock_subprocess, client: TestClient, test_token: str
):
    """测试空文本的文字转语音接口"""
    # 模拟macOS系统
    with patch("platform.system", return_value="Darwin"):
...
@patch("app.api.routers.audio.subprocess.run")
@patch("os.remove")
async def test_text_to_speech_mac_failure(
    mock_remove, mock_subprocess, client: TestClient, test_token: str, mock_tts_text
):
