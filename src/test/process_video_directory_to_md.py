...
def extract_audio_from_video(video_path: str) -> str:
    ...
    temp_audio_path = os.path.join(temp_dir, f"extracted_audio_{os.path.basename(video_path)}.wav")
...
def generate_markdown_output(video_results: list, output_file: str):
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        for i, result in enumerate(video_results):
            if "error" in result:
                continue
            f.write(f"{i+1}. [{result['video_name']}](#{i+1}-{result['video_name'].replace(' ', '-')})\n")
        f.write("\n")
        ...
        for i, result in enumerate(video_results):
            ...
            f.write(f"- 知识点数量: {len(result['knowledge_points'].get('knowledge_points', []))}\n\n")
            ...
            for kp in result['knowledge_points'].get('knowledge_points', []):
                ...
                if timestamp:
                    if level == 1:
                        f.write(f"{indent}- **{kp['title']}** [{timestamp}]: {kp['description']}\n\n")
                    elif level == 2:
                        f.write(f"{indent}- {kp['title']} [{timestamp}]: {kp['description']}\n\n")
                    else:
                        f.write(f"{indent}- {kp['title']} [{timestamp}]: {kp['description']}\n")
        ...
        for result in video_results:
            if "error" in result:
                continue
            all_knowledge_points.extend(result['knowledge_points'].get('knowledge_points', []))
