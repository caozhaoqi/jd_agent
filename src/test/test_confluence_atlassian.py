...
if not CONFLUENCE_USERNAME or not (CONFLUENCE_PASSWORD or CONFLUENCE_API_TOKEN):
    print("错误: 请在.env文件中配置CONFLUENCE_USERNAME和CONFLUENCE_PASSWORD/CONFLUENCE_API_TOKEN")
    sys.exit(1)
...
def test_get_document_history(confluence, page_id):
    ...
    try:
        ...
        if page:
            ...
            print(f"创建者: {page.get('history', {}).get('createdBy', {}).get('displayName')}")
            print(f"最后更新时间: {page.get('version', {}).get('when')}")
            print(f"最后更新者: {page.get('version', {}).get('by', {}).get('displayName')}")
...
def test_get_document_comments(confluence, page_id):
    ...
    try:
        comments = confluence.get_page_child_by_type(
            page_id,
            type='comment'
        )
        ...
        if comments:
            ...
            for i, comment in enumerate(comments[:5]):  # 只显示前5条
                ...
                print(f"内容: {comment.get('body', {}).get('storage', {}).get('value', '')[:200]}...")
