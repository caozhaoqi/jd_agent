...
async def test_upload_resume(client: TestClient, test_user: User, test_token: str, mock_resume_text):
    """测试上传简历接口"""
    # 创建测试文件
...
def test_upload_resume_with_existing_entries(client: TestClient, test_user: User, test_token: str, mock_resume_text, session):
    """测试上传简历接口，当已有部分信息时"""
    # 添加一些已存在的用户资料
    existing_profile = UserProfile(
        user_id=test_user.id,
        category="resume_tech_stack",
        content="Python、Java、Go"
    )
