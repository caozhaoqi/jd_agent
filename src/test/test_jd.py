...
async def test_stream_system_design(
    mock_str_parser,
    ...
    mock_sse_manager.add_connection = AsyncMock(
        return_value=("test_client_id", mock_send_queue)
    )
...
@patch("app.api.routers.jd.generate_interview_guide", new_callable=AsyncMock)
async def test_generate_guide_error(
    mock_generate_interview_guide, client: TestClient, test_token: str, mock_jd_text
):
    """测试生成面试指南失败的情况"""
    # 配置模拟对象抛出异常
...
@pytest.mark.parametrize(
    "years_required, expected", [("3年以上", "3年以上"), (None, "不限"), ("", "")]
)
