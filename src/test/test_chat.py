...
def test_get_sessions(
    client: TestClient, test_user: User, test_session: ChatSession, test_token: str
):
    """测试获取用户的聊天会话列表"""
    response = client.get(
...
def test_get_messages(
    ...
    ai_msg = ChatMessage(
        session_id=test_session.id, role="assistant", content="Hi there!"
    )
...
def test_get_messages_nonexistent_session(
    client: TestClient, test_user: User, test_token: str
):
    """测试获取不存在的会话消息"""
    response = client.get(
