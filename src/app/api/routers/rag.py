...
@router.post("/qa/stream", summary="流式RAG接口", description="基于本地知识库流式回答问题")
async def stream_query_knowledge_base(request: RAGRequest):
    """
    流式RAG接口：基于本地知识库流式回答问题
    """
    try:
        from app.chains.rag_chain import rewrite_chain, format_docs_with_source, extract_sources
        ...
        async def generate():
            try:
                ...
                # 使用LLM的流式生成
                async for chunk in llm.astream(prompt.format(context=context, question=better_question)):
