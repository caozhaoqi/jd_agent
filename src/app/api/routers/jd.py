...
async def create_guide(
    ...
    async def generate_and_stream():
        ...
        async def process_stream_queue():
            ...
            try:
                while True:
                    ...
                    msg = await queue.get()
                    logger.info(f"📥 [Process Queue] 收到队列消息: {msg}, thread_id: {thread_id}")
                    if msg is None:  # 结束信号
                    ...
                    logger.info(f"📤 [Process Queue] 发送消息到前端: {payload}, thread_id: {thread_id}")
        ...
        logger.info(f"🧹 [Generate and Stream] 所有消息发送完成，队列已清除: {thread_id}")
