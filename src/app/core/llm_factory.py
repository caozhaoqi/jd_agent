...
class CachedLLM:
    ...
    def __init__(
        self, llm, cache_expire_seconds: int = settings.CACHE_EXPIRATION_DEFAULT
    ):
        self.llm = llm
    ...
    def _generate_cache_key(self, messages: Any, **kwargs) -> str:
        ...
        def process_message_content(content):
            ...
            if isinstance(content, BaseMessage):
                ...
                return {
                    ...
                    "additional_kwargs": process_message_content(
                        content.additional_kwargs
                    ),
                    "response_metadata": process_message_content(
            ...
            if hasattr(content, "to_json"):
                try:
                    ...
                    logger.debug(
                        f"⚠️ [LLM Cache] to_json() failed: {e}, falling back to direct dict"
                    )
            ...
            if hasattr(content, "dict"):
                try:
                    ...
                    logger.debug(
                        f"⚠️ [LLM Cache] dict() failed: {e}, falling back to __dict__"
                    )
            ...
            if hasattr(content, "__dict__"):
                try:
                    ...
                    logger.debug(
                        f"⚠️ [LLM Cache] __dict__ failed: {e}, falling back to string"
                    )
        ...
        for msg in messages:
            try:
                logger.debug(
                    f"📝 [LLM Cache] Processing message type: {type(msg).__name__}"
                )
                    ...
                    try:
                        if hasattr(msg, "to_json"):
                            ...
                            serialized_messages.append(
                                process_message_content(msg_json)
                            )
                            continue
                    except Exception as e:
                        logger.debug(
                            f"⚠️ [LLM Cache] to_json() failed: {e}, falling back to type+content"
                        )
                ...
                # 检查是否有 type 和 content 属性
                    ...
                    try:
                        ...
                        logger.debug(
                            f"⚠️ [LLM Cache] dict() failed: {e}, falling back to string"
                        )
                ...
                logger.error(
                    f"❌ [LLM Cache] Failed to serialize message: {e}, msg type: {type(msg).__name__}"
                )
                logger.error(f"❌ [LLM Cache] Message details: {msg}")
                # 如果序列化失败，使用字符串表示
                serialized_messages.append({"type": "error", "content": str(msg)})
        ...
        try:
            logger.debug(
                f"🔑 [LLM Cache] Generating cache key with data type: {type(cache_data).__name__}"
            )
            cache_str = json.dumps(cache_data, sort_keys=True)
            cache_key = hashlib.md5(cache_str.encode()).hexdigest()
            logger.debug(
                f"✅ [LLM Cache] Successfully generated cache key: {cache_key}"
            )
    ...
    def __call__(self, messages: List[BaseMessage], **kwargs) -> Any:
        ...
        # 只有非流式调用才使用缓存
        if (
            kwargs.get("streaming", False)
            or hasattr(self.llm, "streaming")
            and self.llm.streaming
        ):
        ...
        if cached_result:
            ...
            try:
                ...
                if isinstance(cached_result, (tuple, list)):
                    logger.debug(
                        f"📝 [LLM Cache] Processing tuple/list cached result: {cached_result}"
                    )
                    # 如果是元组或列表，尝试使用第一个元素
                    if (
                        len(cached_result) > 0
                        and isinstance(cached_result[0], dict)
                        and "message" in cached_result[0]
                    ):
                ...
                if isinstance(cached_result, dict) and "message" in cached_result:
                    ...
                    if isinstance(message_dict, (tuple, list)):
                        logger.debug(
                            f"📝 [LLM Cache] Processing tuple/list message_dict: {message_dict}"
                        )
                        # 如果是元组或列表，尝试使用第一个元素
                        if (
                            len(message_dict) > 0
                            and isinstance(message_dict[0], dict)
                            and "type" in message_dict[0]
                            and "content" in message_dict[0]
                        ):
                        ...
                        if "message" in cached_result:
                            ...
                            if (
                                ...
                                if content is None:
                                    ...
                                    content = str(content)
                                logger.debug(
                                    f"📝 [LLM Cache] Directly extracted content from cache: {content[:100]}..."
                                )
                                return content
                    # 如果无法提取内容，将整个缓存结果转换为字符串
                    logger.debug(
                        f"📝 [LLM Cache] Converting entire cached result to string"
                    )
                    return str(cached_result)
                except Exception as inner_e:
                    logger.error(
                        f"❌ [LLM Cache] Failed to extract or convert cached result: {inner_e}"
                    )
        ...
        try:
            logger.debug(
                f"📝 [LLM Call] messages type: {type(messages).__name__}, messages: {messages}"
            )
                ...
                if isinstance(msg, tuple):
                    ...
                    if len(msg) > 0 and isinstance(msg[0], dict) and "type" in msg[0]:
                        ...
                        try:
                            ...
                            logger.debug(
                                f"❌ [LLM Call] Failed to convert tuple element to BaseMessage: {e}"
                            )
                    # 如果转换失败，将元组转换为字符串
                    ...
                    logger.debug(
                        f"⚠️ [LLM Call] Found non-BaseMessage element: {msg}, type: {type(msg).__name__}"
                    )
                    try:
                        ...
                        logger.debug(
                            f"❌ [LLM Call] Failed to convert element to BaseMessage: {e}"
                        )
        ...
        try:
            ...
            if isinstance(result, ChatResult):
                ...
                cache_data = {
                    ...
                    "generation_info": (
                        ...
                        else str(result.generations[0].generation_info)
                    ),
                    "llm_output": (
                        result.llm_output
                        if isinstance(
                        ...
                        else str(result.llm_output)
                    ),
                }
                redis_client.set(cache_key, cache_data, self.cache_expire_seconds)
                logger.info(
                    f"💾 [LLM Cache] Saved result to cache for key: {cache_key}"
                )
            ...
            try:
                ...
                logger.error(
                    f"❌ [LLM Cache] Failed to convert error result to string: {inner_e}"
                )
                # 最后尝试直接转换为字符串
                return str(result)
