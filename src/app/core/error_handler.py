...
def raise_unauthorized(
    ...
    """
    raise_api_error(
        status_code=401, code=ErrorCode.UNAUTHORIZED, message=message, details=details
    )


def raise_forbidden(
    message: str = "访问被拒绝", details: Optional[Dict[str, Any]] = None
) -> None:
    """
    抛出禁止访问错误
    ...
    """
    raise_api_error(
        status_code=403, code=ErrorCode.FORBIDDEN, message=message, details=details
    )


def raise_not_found(
    message: str = "资源不存在", details: Optional[Dict[str, Any]] = None
) -> None:
    """
    抛出资源不存在错误
...
def raise_validation_error(
    message: str = "请求参数验证失败", details: Optional[Dict[str, Any]] = None
) -> None:
    """
    抛出参数验证错误
...
def handle_exception(
    ...
    return APIException(
        status_code=status_code, code=code, message=message, details={"error": str(exc)}
    )
