...
def get_stream_queue(thread_id: Optional[str] = None) -> Optional[asyncio.Queue]:
    ...
    if thread_id:
        ...
        logger.info(
            f"📤 [Stream Manager] 获取队列: thread_id={thread_id}, 队列存在={queue is not None}, 当前队列数量: {len(_msg_queues)}, 所有队列ID: {list(_msg_queues.keys())}"
        )
        return queue
    if _queue_accessor:
        return _queue_accessor()
    logger.info(
        f"📤 [Stream Manager] 获取队列失败: thread_id=None, queue_accessor={_queue_accessor is not None}"
    )
    return None
...
async def send_thought(
    ...
    if q:
        ...
        logger.info(
            f"📤 [Stream Manager] 发送思考过程: {step_title}, thread_id: {thread_id}"
        )
