    ...
    async def start_heartbeat(self, interval: int = 30):
        ...
        while self.is_connected:
            try:
                ...
                logger.error(
                    f"💓 [SSE Heartbeat] Error for client {self.client_id}: {e}"
                )
                break
...
class SSEManager:
    ...
    def start_cleanup_task(self):
        ...
        self.cleanup_task.add_done_callback(
            lambda task: (
                logger.error(f"🧹 [SSE Cleanup] Task failed: {task.exception()}")
                if task.exception()
                else None
            )
        )
    ...
    async def _cleanup_expired_connections(self):
        ...
        async with self.lock:
            ...
            for client_id in expired_ids:
                logger.info(
                    f"🧹 [SSE Cleanup] Removing expired connection: {client_id}"
                )
    ...
    async def add_connection(self) -> (str, asyncio.Queue):
        ...
        logger.info(
            f"➕ [SSE Manager] Added new connection: {client_id} (Total: {len(self.connections)})"
        )
    ...
    async def remove_connection(self, client_id: str):
        ...
        async with self.lock:
            if client_id in self.connections:
                ...
                await conn.close()
                logger.info(
                    f"➖ [SSE Manager] Removed connection: {client_id} (Total: {len(self.connections)})"
                )
        ...
        async with self.lock:
            for client_id, conn in self.connections.items():
                try:
                    ...
                    logger.error(
                        f"📢 [SSE Broadcast] Error sending to client {client_id}: {e}"
                    )
