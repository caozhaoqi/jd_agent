...
async def jd_parser_node(state: AgentState, config: Optional[dict] = None):
    ...
    # ✅ 记录复杂度和智能体选择结果
    await send_thought(f"📊 JD复杂度: {complexity_level}", f"激活智能体: {', '.join(active_agents)}", thread_id)
...
async def researcher_node(state: AgentState, config: Optional[dict] = None):
    ...
    await send_thought(f"🕵️ [Researcher] 正在背调: {state.get('company_name')}", thread_id=thread_id)
    # ✅ 触发 Dashboard 更新
...
async def human_approval_node(state: AgentState, config: Optional[dict] = None):
    ...
    if state.get("human_feedback"):
        ...
        await send_thought(f"✅ 已应用人工反馈", state['human_feedback'], thread_id)
