...
class ConfluenceCrawler:
    ...
    def get_pages_in_space(self, space_key: str, depth: int = 2) -> List[Dict[str, Any]]:
        """获取指定空间的所有页面"""
        try:
            # 直接获取空间所有页面
    ...
    def extract_page_info(self, page: Dict[str, Any]) -> Optional[ConfluencePage]:
        ...
        try:
            ...
            space_name = page.get("space", {}).get("name", "") or page.get("_expandable", {}).get("space", "").split("/")[-1]
            ...
            # 获取作者信息
            created_by = page.get("history", {}).get("createdBy", {}).get("displayName", "未知")
            updated_by = page.get("history", {}).get("lastUpdatedBy", {}).get("displayName", "未知")
    ...
    def crawl_space(self, space_key: str) -> int:
        ...
        for page in pages:
            ...
            if confluence_page:
                ...
                processed_count += 1
                logger.debug(f"✅ 已保存页面: {confluence_page.title} (ID: {confluence_page.page_id})")
